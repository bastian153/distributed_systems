...
main(int argc, char* argv[])
{
    ...
    if(processId == 0) {
         ...
         std::string fileLine;
         int i = 0;
         while(std::getline(openFile, fileLine) && (i / LENGTH) < 100000){
             int j;
    ...
    MPI_Gather(&processMax.lineNumber, 1, MPI_INT, 
               &lineNumberList, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&processMax.firstChar, 1, MPI_INT,
               &firstCharList, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&processMax.length, 1, MPI_INT,
               &lengthList, 1, MPI_INT, 0, MPI_COMM_WORLD);
